...
const MessagePopup = ({
  ...
  console.log({productData,shopName},"Rgtrthrthrhrh")
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      const matchingDocument = newMessages?.filter((doc) => {
        return (
          ...
          doc?.user ===  receiverId &&
          doc?.productId == productData?.product_id &&
          doc?.orderId == orderId
        );
      });
    ...
    if (input.trim() || files.length > 0) {
      ...
      const matchingDocument = documents?.find((doc) => {
        return (
          ...
          doc.data.productId == productData?.product_id &&
      ...
      if (matchingDocument) {
        ...
        await addDoc(collection(db, "chatRooms"), {
          ...
          productData: {
            orderId: orderId,
            name: productData?.productMain?.product_title,
            qty: productData?.qty,
          ...
          >
            <Box sx={{ display: "flex", alignItems: "center" }}>
              <Typography component="div">
                <img
                  src={userImage ? `${userImage}` : "https://i.etsystatic.com/isla/24ec0e/34844512/isla_75x75.34844512_ke6bg9xj.jpg?version=0"}
                  style={{
                    borderRadius: "4px",
                    width: "40px",
                    height: "40px",
