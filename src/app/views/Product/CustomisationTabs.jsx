...
import {
    Autocomplete, Box, Button, Chip, InputLabel, MenuItem, Select, Typography
} from "@mui/material";
...
const CustomisationTabs = ({
    ...
    React.useEffect(() => {
        ...
        EdtiFroemData?.material?.forEach((item) => {
            ...
            combinedMaterials?.forEach((gender) => {
                if (item == gender.label) {
                    setFormData((prev) => ({
                        ...prev, combinedMaterials: [...prev.combinedMaterials, gender]
                    }));
                }
            });
        });
    ...
    const handleAddKey = () => {
        if (formData.searchTerms.trim() && !keys.includes(formData.searchTerms.trim())) {
            ...
            setFormData((prev) => ({
                ...prev, serchTemsKeyArray: [...prev.serchTemsKeyArray, newKey]
            }));
        }
    };

// In CustomisationTabs.jsx, update the handleRowReorder function:

    const handleRowReorder = (combindex, sourceIndex, targetIndex) => {
        console.log('Reordering in parent:', { combindex, sourceIndex, targetIndex });
    ...
    const handleDelete = (keyToDelete) => () => {
        setKeys((keys) => {
            ...
            setFormData((prev) => ({
                ...prev, serchTemsKeyArray: updatedKeys
            }));
    ...
    const handleVariantChange = (variantId, value) => {
        ...
        setFormData((prev) => {
            ...
            return {
                ...prev,
                ParentMainId: value ? prev.ParentMainId.includes(variantId) ? prev.ParentMainId : [...prev.ParentMainId, variantId] : prev.ParentMainId.filter((id) => id !== variantId),
                varientName: value ? [...updatedVarientName, value._id] : updatedVarientName,
            };
        });
    ...
    React.useEffect(() => {
        setFormData((prv) => ({
            ...prv, launchData: selectedLaunchDate, releaseDate: selectedReleasDate
        }));
            ...
            EdtiFroemData?.occasion?.forEach((item) => {
                ...
                OccassionList?.forEach((occasion) => {
                    if (item == occasion._id) {
                        ...
                        setFormData((prev) => ({
                            ...prev, productdetailsOccassion: [...prev.productdetailsOccassion, occasion]
                        }));
                    }
                });
            });
    ...
    const handleToggle = (combindex, index) => {
        setCombinations((prev) => prev?.map((item, i) => i === combindex ? {
            ...item,
            ...
            return prev?.map((comb, i) => {
                ...
                const updatedCombinations = comb.combinations?.map((item, j) => {
                    ...
                    if (type.startsWith('main_images[')) {
                        ...
                        updatedItem = {
                            ...updatedItem, main_images: newMainImages
                        };
            ...
            return prev?.map((comb, i) => {
                ...
                const updatedCombinations = comb.combinations?.map((item, j) => {
                    ...
                    if (type.startsWith('main_images[')) {
                        ...
                        return {
                            ...item, main_images: newMainImages
                        };
                    } else {
          ...
          >
            ...
            /> */}
              ...
              {" "}
                    ...
                    <Box
                        sx={{
                            width: "100%", maxWidth: "100%", display: "flex", gap: "20px"
                        }}
                    >
                        <Box
                            sx={{
              ...
              {" "}
                ...
                {combinations?.length > 0 && (combinations?.map((comb, combindex) => (<Box key={combindex}>
                                ...
                                <Table>
                                    <TableHead>
                                        <TableRow>
                                            ...
                                            {comb.combinations[0]?.name2 && <TableCell
                                                sx={{
                                                    wordBreak: "keep-all"
                                                }}
                                                align="center"
                                    ...
                                    {seletedVariant?.name ? (<Box
                                        ...
                                        >
                                            ...
                                            <Autocomplete
                                                ...
                                                value={item?.variant_attribute?.find((option) => formData?.varientName?.includes(option._id)) || null}
                                                name="variantName"
                                            />
                ...
                <Box
                    sx={{
                        display: "flex", alignItems: "center", justifyContent: "flex-end", marginTop: "20px"
                    }}
                >
                    {/* <div>
            <Button
