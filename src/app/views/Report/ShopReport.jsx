...
const names = ["Shop Name", "Report Type"];
...
const ShopReport = () => {
  ...
  const [personName, setPersonName] = useState(
    JSON.parse(localStorage.getItem(localStorageKey.shopReport)) || []
  );
  ...
  const handleChange = (event) => {
    ...
    localStorage.setItem(localStorageKey.shopReport, JSON.stringify(setPerson));
    if (setPerson.length <= 0) {
      localStorage.removeItem(localStorageKey.shopReport);
    }
  };
  ...
  const getreportList = useCallback(async () => {
    try {
      const res = await ApiService.get(apiEndpoints.getShopReports, auth_key);

      if (res.status === 200) {
        console.log({ res });
        const myNewList = res?.data?.storeReport?.map((e, i) => {
          return { "S.No": i + 1, ...e };
        });
        ...
        const xData = myNewList.map((e, i) => {
          let obj = {
            "S.NO": i + 1,
            "Shop Name": e?.vendorDetails?.storedetail?.shop_name || e?.vendorDetails?.name,
            "Report Type": e?.type
          };
          return obj;
        });
  ...
  return (
    <Box sx={{ margin: "30px" }}>
      ...
      >
        <Breadcrumb routeSegments={[{ name: "Report", path: "" }, { name: "Shop Report" }]} />
                  ...
                  return (
                    <TableRow key={row._id}>
                      ...
                      {!personName?.includes("Image") && (
                        <TableCell>
                          <img
                            src={`${shopBaseUrl}${row?.vendorDetails?.storedetail?.shop_icon}`}
                            alt="Product"
                      ...
                      {!personName?.includes("Shop Name") && (
                        <TableCell>
                          {capitalizeFirstLetter(
                            row?.vendorDetails?.storedetail?.shop_name ||
                      ...
                      <TableCell>
                        <IconButton
                          onClick={() =>
                            navigate(`${ROUTE_CONSTANT.report.viewShopReport}?id=${row._id}`)
                          }
                        >
