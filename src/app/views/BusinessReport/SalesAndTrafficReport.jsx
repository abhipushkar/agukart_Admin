...
const SalesAndTrafficReport = () => {
  ...
  const [personName, setPersonName] = useState(
    JSON.parse(localStorage.getItem(localStorageKey.SalesAndTrafficReportTable)) || []
  );
  ...
  const handleChange = (event) => {
    ...
    localStorage.setItem(localStorageKey.SalesAndTrafficReportTable, JSON.stringify(setPerson));
    if (setPerson.length <= 0) {
      localStorage.removeItem(localStorageKey.SalesAndTrafficReportTable);
    }
  };
  ...
  const getreportList = useCallback(async () => {
    try {
      const res = await ApiService.get(
        `${apiEndpoints.getSalesAndTraficReport}?startDate=${date?.from}&endDate=${date?.to}&wiseType=${date?.groupBy}`,
        auth_key
      );
                ...
                sortedRows.map((row, i) => {
                  return (
                    <TableRow key={row._id}>
                      ...
                      {!personName?.includes("Total Views Items") && (
                        <TableCell sx={{ wordBreak: "break-word" }}>{row.totalViewItems}</TableCell>
                      )}
                      {!personName?.includes("Total Order Items") && (
                      ...
                      {!personName?.includes("Units Ordered") && (
                        <TableCell sx={{ wordBreak: "break-word" }}>{row.unitsOrdered}</TableCell>
                      )}
                      {!personName?.includes("Ordered products sales") && (
                        <TableCell sx={{ wordBreak: "break-word" }}>
                      ...
                      {!personName?.includes("Amount refunded") && (
                        <TableCell sx={{ wordBreak: "break-word" }}>{row?.amountRefunded?.toFixed(2)}</TableCell>
                      )}
                      {!personName?.includes("No. of cancel") && (
                        <TableCell sx={{ wordBreak: "break-word" }}>{row.numberOfCancel}</TableCell>
                      )}
                      {!personName?.includes("Conversion Rate") && (
...
export default SalesAndTrafficReport;
