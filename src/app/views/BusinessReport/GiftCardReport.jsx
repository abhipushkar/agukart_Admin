...
const names = ["Date","Total View Items","Total Order Gift Card","Gift Card Ordered","Ordered Gift Card Sales"];

const label = { inputProps: { "aria-label": "Switch demo" } };

const GiftCardReport = () => {
  ...
  const [personName, setPersonName] = useState(
    JSON.parse(localStorage.getItem(localStorageKey.GiftCardReportTable)) || []
  );
  ...
  const handleChange = (event) => {
    ...
    localStorage.setItem(localStorageKey.GiftCardReportTable, JSON.stringify(setPerson));
    if (setPerson.length <= 0) {
      localStorage.removeItem(localStorageKey.GiftCardReportTable);
    }
  };
  ...
  const getreportList = useCallback(async () => {
    try {
      const res = await ApiService.get(
        `${apiEndpoints.getGiftCardReport}?startDate=${date.from}&endDate=${date.to}&wiseType=${date?.groupBy}`,
        auth_key
      );
      ...
      if (res.status === 200) {
        ...
        const xData = myNewList.map((e, i) => {
          let obj = {
            ...
            "Total Order Gift Card": e.totalOrderGiftCard,
            "Gift Card Ordered": e.giftCardOrdered,
            "Ordered Gift Card Sales": e.orderedGiftCardSales
          };
          return obj;
        });
        setExcelData(xData);
...
const formatDateCell = (dateStr, groupBy) => {
  ...
  return (
    <Box sx={{ margin: "30px" }}>
      ...
      >
        <Breadcrumb
          routeSegments={[{ name: "Business Report", path: "" }, { name: "Gift Cards Report" }]}
        />
        <Box
          sx={{
            display: "flex",
          ...
          >
            <TableHead>
              <TableRow>
                ...
                {!personName?.includes("Total Order Gift Card") && (
                  <TableCell sortDirection={orderBy === "totalOrderGiftCard" ? order : false}>
                    <TableSortLabel
                      active={orderBy === "totalOrderGiftCard"}
                      direction={orderBy === "totalOrderGiftCard" ? order : "asc"}
                      onClick={() => handleRequestSort("totalOrderGiftCard")}
                    >
                      Total Order Gift Card
                    </TableSortLabel>
                  </TableCell>
                )}
                {!personName?.includes("Gift Card Ordered") && (
                  <TableCell sortDirection={orderBy === "giftCardOrdered" ? order : false}>
                    <TableSortLabel
                      active={orderBy === "giftCardOrdered"}
                      direction={orderBy === "giftCardOrdered" ? order : "asc"}
                      onClick={() => handleRequestSort("giftCardOrdered")}
                    >
                      Gift Card Ordered
                    </TableSortLabel>
                  </TableCell>
                )}
                {!personName?.includes("Ordered Gift Card Sales") && (
                  <TableCell sortDirection={orderBy === "title" ? order : false}>
                    <TableSortLabel
                      active={orderBy === "orderedGiftCardSales"}
                      direction={orderBy === "orderedGiftCardSales" ? order : "asc"}
                      onClick={() => handleRequestSort("orderedGiftCardSales")}
                    >
                      Ordered Gift Card Sales
                    </TableSortLabel>
                  </TableCell>
                )}
              </TableRow>
                ...
                sortedRows.map((row, i) => {
                  return (
                    <TableRow key={row._id}>
                      ...
                      {!personName?.includes("Ordered Gift Card Sales") && (
                        <TableCell sx={{ wordBreak: "break-word" }}>
                          {row?.orderedGiftCardSales?.toFixed(2)}
                        </TableCell>
                      )}
                    </TableRow>
                  );
