...
const NewUserReport = () => {
  ...
  const [personName, setPersonName] = useState(
    JSON.parse(localStorage.getItem(localStorageKey.NewUsersReportTable)) || []
  );
  ...
  const handleChange = (event) => {
    ...
    localStorage.setItem(localStorageKey.NewUsersReportTable, JSON.stringify(setPerson));
    if (setPerson.length <= 0) {
      localStorage.removeItem(localStorageKey.NewUsersReportTable);
    }
  };
  ...
  const getreportList = useCallback(async () => {
    try {
      const res = await ApiService.get(`${apiEndpoints.getNewUserReport}?startDate=${date.from}&endDate=${date.to}&wiseType=${date?.groupBy}`, auth_key);
  ...
  return (
    <Box sx={{ margin: "30px" }}>
      ...
      >
        <Breadcrumb
          routeSegments={[{ name: "Business Report", path: "" }, { name: "New Users Report" }]}
        />
        <Box
          sx={{
            display: "flex",
          ...
          >
            <TableHead>
              <TableRow>
                ...
                {!personName?.includes("No. Of Users") && (
