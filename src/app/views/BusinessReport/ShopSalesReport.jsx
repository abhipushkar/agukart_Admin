...
const ShopSalesReport = () => {
  ...
  const [personName, setPersonName] = useState(
    JSON.parse(localStorage.getItem(localStorageKey.ShopSaleReportTable)) || []
  );
  ...
  const handleChange = (event) => {
    ...
    localStorage.setItem(localStorageKey.ShopSaleReportTable, JSON.stringify(setPerson));
    if (setPerson.length <= 0) {
      localStorage.removeItem(localStorageKey.ShopSaleReportTable);
    }
  };
  ...
  const getreportList = useCallback(async () => {
    try {
      const res = await ApiService.get(
        `${apiEndpoints.getShopSaleReport}?startDate=${date.from}&endDate=${date.to}&wiseType=${date?.groupBy}`,
        auth_key
      );

      if (res.status === 200) {
        console.log({ res });
        const myNewList = res?.data?.finalData?.[0]?.rawData?.map((e, i) => {
          return { "S.No": i + 1, ...e, date:res?.data?.finalData?.[0]?.date };
        });
        ...
        const xData = myNewList.map((e, i) => {
          let obj = {
            "S.NO": i + 1,
            "Date": e?.date,
            "Shop Name": e?.vendorName,
            "Total Views Items": e?.totalViewItems,
            "Total Order Items": e?.totalOrderItems,
            "Unit Orders": e?.unitsOrdered,
            "ordered product sales": e?.orderedProductSales,
            "Amount refund": e?.amountRefund,
            "No. of cancel": e?.numberOfCancel,
  ...
  return (
    <Box sx={{ margin: "30px" }}>
      ...
      >
        <Breadcrumb
          routeSegments={[{ name: "Business Report", path: "" }, { name: "Shop Sale Report" }]}
        />
        <Box
          sx={{
            display: "flex",
            ...
            <TableHead>
              <TableRow>
                ...
                {!personName?.includes("Amount refunded") && (
                  <TableCell sortDirection={orderBy === "amountRefunded" ? order : false}>
                    <TableSortLabel
                      active={orderBy === "amountRefunded"}
                      direction={orderBy === "amountRefunded" ? order : "asc"}
                      onClick={() => handleRequestSort("amountRefunded")}
                    >
                      Amount Refuned
                    </TableSortLabel>
                  </TableCell>
                ...
                {!personName?.includes("No. of cancel") && (
                  <TableCell sortDirection={orderBy === "numberOfCancel" ? order : false}>
                    <TableSortLabel
                      active={orderBy === "numberOfCancel"}
                      direction={orderBy === "numberOfCancel" ? order : "asc"}
                      onClick={() => handleRequestSort("numberOfCancel")}
                    >
                      No. Of Cancel
                    </TableSortLabel>
                  </TableCell>
                )}
                {!personName?.includes("Conversion Rate") && (
                ...
                sortedRows.map((row, i) => {
                  return (
                    <TableRow key={row._id}>
                      ...
                      {!personName?.includes("Shop Name") && (
                        <TableCell sx={{ wordBreak: "break-word" }}>{row.vendorName}</TableCell>
                      )}
                      {!personName?.includes("Total Views Items") && (
                        <TableCell sx={{ wordBreak: "break-word" }}>{row.totalViews}</TableCell>
                      )}
                      {!personName?.includes("Total Order Items") && (
                      ...
                      {!personName?.includes("Units Ordered") && (
                        <TableCell sx={{ wordBreak: "break-word" }}>{row.totalUnits}</TableCell>
                      )}
                      {!personName?.includes("Ordered products sales") && (
                        <TableCell sx={{ wordBreak: "break-word" }}>
                      ...
                      {!personName?.includes("Amount refunded") && (
                        <TableCell sx={{ wordBreak: "break-word" }}>{row?.totalRefunds?.toFixed(2)}</TableCell>
                      )}
                      {!personName?.includes("No. of cancel") && (
                        <TableCell sx={{ wordBreak: "break-word" }}>{row.cancelledOrders}</TableCell>
                      )}
                      {!personName?.includes("Conversion Rate") && (
                        <TableCell sx={{ wordBreak: "break-word" }}>{row?.conversionRate?.toFixed(2)}</TableCell>
                      )}
                    </TableRow>
                  );
                })
