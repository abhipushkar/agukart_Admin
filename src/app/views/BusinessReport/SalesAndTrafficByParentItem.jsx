...
const names = ["Date","Parent SKU","Parent Id","Image","Title","Total Views Items","Total Order Items","Units Ordered","Ordered Product Sales","Amount Refund","No. Of Cancel","Conversation Rate"];

const label = { inputProps: { "aria-label": "Switch demo" } };

const SalesAndTrafficByParentItem = () => {
  ...
  const [personName, setPersonName] = useState(
    JSON.parse(localStorage.getItem(localStorageKey.SalesAndTrafficByParentItemReportTable)) || []
  );
  ...
  const handleChange = (event) => {
    ...
    localStorage.setItem(
      localStorageKey.SalesAndTrafficByParentItemReportTable,
      JSON.stringify(setPerson)
    );
    if (setPerson.length <= 0) {
      localStorage.removeItem(localStorageKey.SalesAndTrafficByParentItemReportTable);
    }
  };
  ...
  const getreportList = useCallback(async () => {
    try {
      const res = await ApiService.get(`${apiEndpoints.getChildItems}?startDate=${date.from}&endDate=${date.to}&wiseType=${date?.groupBy}&type=parent`, auth_key);
      ...
      >
        <Breadcrumb
          routeSegments={[
            ...
            { name: "Sales And Traffic By Parent Item" }
          ]}
        />
        <Box
          sx={{
                ...
                sortedRows.map((row, i) => {
                  return (
                    <TableRow key={row._id}>
                      ...
                      {!personName?.includes("Unit Ordered") && (
                        <TableCell sx={{ wordBreak: "break-word" }}>
                          {row.unitsOrdered}
                        </TableCell>
                      )}
                      {!personName?.includes("Order Product Sales") && (
                        <TableCell sx={{ wordBreak: "break-word" }}>
...
export default SalesAndTrafficByParentItem;
